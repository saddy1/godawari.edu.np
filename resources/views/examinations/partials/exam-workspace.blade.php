<section x-data="{ marksOpen: false }" class="overflow-hidden rounded-2xl border border-emerald-200 bg-white shadow-sm">
    <header class="flex flex-col gap-3 bg-gradient-to-r from-emerald-50 to-white p-4 sm:flex-row sm:items-center sm:justify-between"><div><div class="flex flex-wrap gap-1.5"><span class="rounded bg-[#1a5632] px-2 py-1 text-[8px] font-black uppercase text-white">{{$examination->status}}</span><span class="rounded bg-white px-2 py-1 text-[8px] font-black uppercase text-gray-600">{{$examination->category}}</span><span class="rounded bg-blue-50 px-2 py-1 text-[8px] font-black uppercase text-blue-700">{{$examination->scope_type==='organization'?'Whole organization':$examination->departments->count().' selected faculties'}}</span></div><h2 class="mt-2 text-xl font-black">{{$examination->name}}</h2><p class="mt-1 text-[10px] font-semibold text-gray-500">{{$examination->organization->name}} · {{$examination->departments->pluck('name')->implode(', ')}} · {{$examination->sections->count()}} sections</p></div><div class="flex flex-wrap gap-2"><button type="button" @click="marksOpen = !marksOpen" :aria-expanded="marksOpen" aria-controls="exam-subject-marks" class="rounded-lg border border-emerald-200 bg-white px-3 py-2 text-xs font-black text-[#1a5632]" x-text="marksOpen ? 'Hide subjects' : 'Edit marks'">Edit marks</button>@if($canManage)<a href="{{ route('admin.examinations.subject-dates.index', $examination) }}" class="rounded-lg border border-emerald-200 bg-white px-3 py-2 text-xs font-black text-[#1a5632]">Subject dates</a><a href="{{route('admin.examinations.admit-cards.index',$examination)}}" class="rounded-lg border border-gray-200 bg-white px-3 py-2 text-xs font-black text-gray-600">Admit Cards</a><a href="{{route('admin.examinations.marksheets.print',$examination)}}" class="rounded-lg border border-gray-200 bg-white px-3 py-2 text-xs font-black text-gray-600">Marksheets</a>@endif<a href="{{route('admin.examinations.index')}}" class="rounded-lg border bg-white px-3 py-2 text-xs font-black text-gray-600">Close</a>@if($canManage)<button type="button" onclick="document.getElementById('exam-details').showModal()" class="rounded-lg bg-[#1a5632] px-3 py-2 text-xs font-black text-white">Edit setup</button>@endif</div></header>
    <div id="exam-subject-marks" x-show="marksOpen" x-cloak>
        ...
        <div class="flex-1 overflow-y-auto p-5">
            <div class="space-y-5">
                ...
                <div class="border-t border-gray-100 pt-5">
                    ...
                    <div x-data="{practicalEnabled: @js((bool) $examination->practical_enabled)}" @click="practicalEnabled=!practicalEnabled" class="mt-3 flex cursor-pointer items-start gap-3 rounded-xl border border-gray-200 bg-gray-50/60 px-3 py-2.5 text-xs font-semibold text-gray-600">
                        <input type="hidden" name="practical_enabled" :value="practicalEnabled ? 1 : 0">
                        <span :class="practicalEnabled?'bg-[#1a5632] text-white':'border border-gray-300 bg-white text-transparent'" class="mt-0.5 grid h-5 w-5 shrink-0 place-items-center rounded-md text-[10px] font-black transition-colors">✓</span>
                        <span><b class="block font-black text-gray-800">Include practical subjects</b><span class="text-[10px] font-semibold text-gray-400">Untick to treat every subject as theory-only for this exam, even subjects that normally have a practical.</span></span>
                    </div>
                </div>
