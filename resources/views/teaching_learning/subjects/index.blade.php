                            ...
                            <form method="POST" action="{{ route('admin.teaching-learning.section-groups.update') }}" class="space-y-3" @submit="if ($event.submitter?.value === 'save' && !selected.length) { $event.preventDefault(); alert('Select at least one section.'); }">
                                ...
                                <div class="flex flex-col gap-2 lg:flex-row lg:items-end">
                                    <div class="min-w-56 lg:w-64">
                                        ...
                                        <input name="group_name" x-model="groupName" @input.debounce.150ms="loadExistingGroup()" list="section-group-list" required placeholder="e.g. Bio" class="w-full rounded-lg border border-emerald-200 bg-white px-3 py-2 text-sm font-extrabold outline-none transition-colors duration-300 hover:border-emerald-300 focus:border-emerald-600 focus:ring-2 focus:ring-emerald-600/15">
                                                ...
                                                @if($canCreate)
                                                    <form x-show="edit" x-cloak x-transition method="POST" action="{{ route('admin.teaching-learning.subject-offerings.update', $offering) }}" class="mt-3 rounded-xl border border-emerald-200 bg-emerald-50/50 p-3">
                                                        ...
                                                        <div class="grid gap-2.5 sm:grid-cols-2 xl:grid-cols-[minmax(180px,1fr)_130px_90px_150px_170px] xl:items-end">
                                                            <div><label class="mb-1 block text-[10px] font-extrabold uppercase tracking-wider text-gray-500">Subject name</label><input name="subject_name" value="{{ $offering->subject->name }}" required class="w-full rounded-lg border border-gray-200 px-3 py-2 text-xs font-semibold outline-none transition-colors duration-300 hover:border-gray-300 focus:border-[#1a5632] focus:ring-2 focus:ring-[#1a5632]/15"></div>
                                                            <div><label class="mb-1 block text-[10px] font-extrabold uppercase tracking-wider text-gray-500">Theory code</label><input name="subject_code" value="{{ $offering->subject->code }}" required class="w-full rounded-lg border border-gray-200 px-3 py-2 text-xs font-extrabold uppercase outline-none transition-colors duration-300 hover:border-gray-300 focus:border-[#1a5632] focus:ring-2 focus:ring-[#1a5632]/15"></div>
                                                            <div><label class="mb-1 block text-[10px] font-extrabold uppercase tracking-wider text-gray-500">Credits</label><input type="number" name="credit_hours" value="{{ $offering->subject->credit_hours }}" min="0" max="255" class="w-full rounded-lg border border-gray-200 px-3 py-2 text-xs font-semibold outline-none transition-colors duration-300 hover:border-gray-300 focus:border-[#1a5632] focus:ring-2 focus:ring-[#1a5632]/15"></div>
                                                            <div><span class="mb-1 block text-[10px] font-extrabold uppercase tracking-wider text-gray-500">Practical</span><label :class="practical ? 'border-emerald-500 bg-emerald-100 text-emerald-800' : 'border-gray-200 bg-white text-gray-600'" class="flex h-[34px] cursor-pointer items-center gap-2 rounded-lg border px-3 transition-colors duration-300"><input type="checkbox" name="has_practical" value="1" x-model="practical" class="sr-only"><span :class="practical ? 'border-emerald-600 bg-emerald-600 text-white' : 'border-gray-300 text-transparent'" class="flex h-4 w-4 items-center justify-center rounded border-2"><svg class="h-3 w-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7"/></svg></span><span class="text-[11px] font-bold" x-text="practical ? 'Yes' : 'No'"></span></label></div>
                                                            <div x-show="practical"><label class="mb-1 block text-[10px] font-extrabold uppercase tracking-wider text-emerald-700">Practical code</label><input name="practical_code" value="{{ $offering->subject->practical_code }}" :disabled="!practical" placeholder="Same as theory" class="w-full rounded-lg border border-emerald-200 px-3 py-2 text-xs font-extrabold uppercase outline-none transition-colors duration-300 hover:border-emerald-300 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/15 disabled:border-gray-100 disabled:bg-gray-50 disabled:text-gray-400"></div>
                                                        </div>
                                                        ...
                                                        <div class="mt-2.5 grid gap-2.5 sm:grid-cols-2 xl:grid-cols-[140px_minmax(170px,1fr)_150px_minmax(150px,1fr)_auto_auto] xl:items-end">
                                                            @if($selectedDept->academic_system === 'semester')
                                                                <div><label class="mb-1 block text-[10px] font-extrabold uppercase tracking-wider text-gray-500">Semester</label><select name="semester" class="w-full rounded-lg border border-gray-200 px-3 py-2 text-xs font-semibold outline-none transition-colors duration-300 hover:border-gray-300 focus:border-[#1a5632] focus:ring-2 focus:ring-[#1a5632]/15"> <option value="">All semesters</option>@for($semester = 1; $semester <= 8; $semester++)<option value="{{ $semester }}" @selected((int)$offering->semester === $semester)>Semester {{ $semester }}</option>@endfor</select></div>
                                                            @elseif($selectedDept->academic_system === 'year')
                                                                <div><label class="mb-1 block text-[10px] font-extrabold uppercase tracking-wider text-gray-500">Study year</label><select name="year_level" class="w-full rounded-lg border border-gray-200 px-3 py-2 text-xs font-semibold outline-none transition-colors duration-300 hover:border-gray-300 focus:border-[#1a5632] focus:ring-2 focus:ring-[#1a5632]/15"><option value="">All years</option>@for($year = 1; $year <= 4; $year++)<option value="{{ $year }}" @selected((int)$offering->year_level === $year)>Year {{ $year }}</option>@endfor</select></div>
                                                            @else
                                                            ...
                                                            @endif
                                                            <div><label class="mb-1 block text-[10px] font-extrabold uppercase tracking-wider text-gray-500">Class group</label><select name="group_name" class="w-full rounded-lg border border-gray-200 px-3 py-2 text-xs font-semibold outline-none transition-colors duration-300 hover:border-gray-300 focus:border-[#1a5632] focus:ring-2 focus:ring-[#1a5632]/15"><option value="">Whole faculty / class</option>@foreach($deptGroups as $group)<option value="{{ $group }}" @selected($offering->group_name === $group)>{{ $group }}</option>@endforeach</select></div>
                                                            <div><span class="mb-1 block text-[10px] font-extrabold uppercase tracking-wider text-gray-500">Subject type</span><label :class="elective ? 'border-purple-500 bg-purple-50 text-purple-800' : 'border-gray-200 bg-white text-gray-600'" class="flex h-[34px] cursor-pointer items-center gap-2 rounded-lg border px-3 transition-colors duration-300"><input type="checkbox" name="is_elective" value="1" x-model="elective" class="sr-only"><span :class="elective ? 'border-purple-600 bg-purple-600 text-white' : 'border-gray-300 text-transparent'" class="flex h-4 w-4 items-center justify-center rounded border-2"><svg class="h-3 w-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7"/></svg></span><span class="text-[11px] font-bold" x-text="elective ? 'Elective' : 'Fixed'"></span></label></div>
                                                            <div><label class="mb-1 block text-[10px] font-extrabold uppercase tracking-wider text-gray-500">Elective group</label><input name="elective_group" value="{{ $offering->elective_group }}" :disabled="!elective" placeholder="e.g. Elective I" class="w-full rounded-lg border border-gray-200 px-3 py-2 text-xs font-semibold outline-none transition-colors duration-300 hover:border-gray-300 focus:border-purple-500 focus:ring-2 focus:ring-purple-500/15 disabled:border-gray-100 disabled:bg-gray-100 disabled:text-gray-400"></div>
