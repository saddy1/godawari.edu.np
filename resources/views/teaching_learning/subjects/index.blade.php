                        ...
                        <div class="border-b border-gray-100 p-3 sm:p-4">
                            ...
                            <form method="POST" action="{{ route('admin.teaching-learning.subject-offerings.store') }}" class="rounded-xl border border-gray-200 bg-gray-50/70 p-3">
                                ...
                                <div x-show="existingSubject" x-cloak class="mt-2.5 flex flex-wrap items-center gap-2 rounded-lg border border-emerald-200 bg-emerald-50 px-3 py-2 text-xs">
                                    ...
                                    <span class="rounded bg-white px-2 py-0.5 text-[10px] font-bold" :class="existingSubject?.has_practical ? 'text-emerald-700' : 'text-gray-500'" x-text="existingSubject?.has_practical ? 'Practical' : 'Theory'"></span>
                                </div>

                                <div x-show="isNewSubject" x-cloak class="mt-2.5 rounded-lg border border-amber-200 bg-amber-50 p-3">
                                    <div class="grid gap-2.5 sm:grid-cols-2 lg:grid-cols-[1fr_130px_170px] lg:items-end">
                                ...
                                <section class="overflow-hidden rounded-2xl border border-gray-200">
                                    ...
                                    <div class="divide-y divide-gray-100">
                                        @foreach($groupOfferings as $offering)
                                            <div class="group flex flex-col gap-3 px-4 py-3.5 sm:flex-row sm:items-center sm:justify-between sm:px-5">
                                                <div class="flex min-w-0 items-center gap-3"><div class="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-emerald-50 text-xs font-black text-[#1a5632]">{{ strtoupper(substr($offering->subject->code, 0, 2)) }}</div><div class="min-w-0"><p class="truncate text-sm font-extrabold text-gray-900">{{ $offering->subject->name }}</p><div class="mt-1 flex flex-wrap items-center gap-1.5"><span class="rounded-md bg-gray-100 px-2 py-0.5 text-[10px] font-extrabold text-gray-500">{{ $offering->subject->code }}</span>@if($offering->subject->has_practical)<span class="rounded-md bg-emerald-50 px-2 py-0.5 text-[10px] font-extrabold text-emerald-700">Practical</span>@endif @if($offering->is_elective)<span class="rounded-md bg-purple-50 px-2 py-0.5 text-[10px] font-extrabold text-purple-700">Elective{{ $offering->elective_group ? ': '.$offering->elective_group : '' }}</span>@else<span class="rounded-md bg-blue-50 px-2 py-0.5 text-[10px] font-extrabold text-blue-700">Fixed</span>@endif</div></div></div>
                                                @if($canDelete)<form method="POST" action="{{ route('admin.teaching-learning.subject-offerings.destroy', $offering) }}" onsubmit="return confirm('Remove this subject from the selected faculty?')">@csrf @method('DELETE')<button type="submit" class="inline-flex items-center gap-1.5 self-end rounded-lg border border-red-100 bg-red-50 px-3 py-2 text-xs font-extrabold text-red-600 transition hover:border-red-200 hover:bg-red-100"><svg class="h-3.5 w-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 12H4"/></svg>Remove</button></form>@endif
                                            </div>
                                        @endforeach
                                    </div>
                                </section>
